...
public:
    ...
    ~TestUniformBufferDynamic() {
        vkDeviceWaitIdle(dev);
        delete index_buf; delete vertex_buf; delete uniform_buf;
    ...
    void CreateBuffer() {
        ...
        struct XXX {
            uint8_t padding[256]; // demostrate dynamic buffer offset
            MVP ubo {};
        } xxx;
        xxx.ubo.model = glm::mat4(1.0f);
        xxx.ubo.view = glm::lookAt(glm::vec3(0.0, 0.0, -10.0), glm::vec3(0.0), glm::vec3(0.0, 1.0, 0.0));
        xxx.ubo.proj = glm::perspective(glm::radians(45.0), (double)m_width/m_height, 0.1, 10.0);

        uniform_buf = new GfxBuffer(this);
        uniform_buf->init(sizeof(XXX), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        uniform_buf->host_update(&xxx, sizeof(XXX));
        uniform_buf->set_descriptor_info(0, sizeof(MVP));
    }
    void CreateDescriptor() {
        // descriptor set layout
        std::vector<VkDescriptorSetLayoutBinding> bindings = {
            GfxDescriptorSetLayoutBinding(0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1, VK_SHADER_STAGE_VERTEX_BIT),
        };
        ...
        std::vector<VkDescriptorPoolSize> pool_size = {
            GfxDescriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1)
        };
        ...
        std::vector<VkWriteDescriptorSet> w = {
            GfxWriteDescriptorSet(ds, 0, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, &uniform_buf->get_descriptor_info())
        };
    ...
    void BakeCommand() {
        ...
        for (uint32_t i = 0; i < cmdbuf.size(); i++) {
            ...
            std::vector<uint32_t> dynamic_buf_offset;
            dynamic_buf_offset.push_back(256*sizeof(uint8_t));
            vkCmdBindDescriptorSets(cmdbuf[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1, &ds, dynamic_buf_offset.size(), dynamic_buf_offset.data());
    ...
    void CreatePipeline() {
        auto vert = loadSPIRV(dev, "uniform.vert.spv");
    ...
    GfxBuffer *uniform_buf;
};

}
int main() {
    common::TestUniformBufferDynamic T;
    T.Gameloop();
    return 0;
}
