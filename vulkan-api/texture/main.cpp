...
class TestTexture : public SkeletonVulkan {
public:
    TestTexture() : SkeletonVulkan(VK_API_VERSION_1_0, 800, 800) {
        SkeletonVulkan::init();
        {
    ...
    ~TestTexture() {
        ...
        delete index_buf; delete vertex_buf; delete texture;
    ...
    void CreateTexture() {
        ...
        texture->bake("../resource/buu.png", VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
        texture->transition(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT); 
#if USE_SEPARATE_IMAGE_SAMPLER
    ...
    void CreateDescriptor() {
        ...
        std::vector<VkDescriptorSetLayoutBinding> bindings = {
            ...
            GfxDescriptorSetLayoutBinding(1, VK_DESCRIPTOR_TYPE_SAMPLER, 1, VK_SHADER_STAGE_FRAGMENT_BIT)
#else
...
#if USE_SEPARATE_IMAGE_SAMPLER
            GfxWriteDescriptorSet(ds, 0, 1, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, &texture->get_descriptor_info()),
            GfxWriteDescriptorSet(ds, 1, 1, VK_DESCRIPTOR_TYPE_SAMPLER, &smp_info),
#else
...
#if USE_SEPARATE_IMAGE_SAMPLER
        auto frag = loadSPIRV(dev, "separate_image_sampler.frag.spv");
#else
