...
class TestInputattachment : public SkeletonVulkan {
public:
    TestInputattachment() : SkeletonVulkan(VK_API_VERSION_1_0, 800, 800) {
        SkeletonVulkan::init();
        {
    ...
    void CreateTexture() {
        ...
        src_texture->bake("../resource/piccolo.png", VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
        ...
        dst_texture->set_descriptor_info(VK_NULL_HANDLE, VK_IMAGE_LAYOUT_GENERAL);
    }
    void CreateDescriptor() {
        {
            // Subpass 0 descriptor set layout
        ...
        {
            // Subpass 1 descriptor set layout
            std::vector<VkDescriptorSetLayoutBinding> bindings = {
                GfxDescriptorSetLayoutBinding(0, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 1, VK_SHADER_STAGE_FRAGMENT_BIT)
            };
            VkDescriptorSetLayoutCreateInfo dsl_info = GfxDescriptorSetLayoutCreateInfo(bindings);
            vkCreateDescriptorSetLayout(dev, &dsl_info, nullptr, &dsl[1]);

            // Subpass 1 pipeline layout
        ...
        std::vector<VkDescriptorPoolSize> pool_size = {
            GfxDescriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1),
            GfxDescriptorPoolSize(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, 1)
        };
        ...
        {
            std::vector<VkWriteDescriptorSet> w = {
                GfxWriteDescriptorSet(ds[1], 0, 1, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, &dst_texture->get_descriptor_info())
            };
            vkUpdateDescriptorSets(dev, w.size(), w.data(), 0, nullptr);
        }
    }
    void BakeCommand() {
        ...
        for (uint32_t i = 0; i < cmdbuf.size(); i++) {
            ...
            vkCmdBindDescriptorSets(cmdbuf[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout[0], 0, 1, &ds[0], 0, nullptr);
    ...
    void CreateRenderPass() {
        ...
        sp_desc[1].inputAttachmentCount = 1;
        sp_desc[1].pInputAttachments = &att_ref[0];
    ...
    void CreatePipeline() {
        ...
        auto sp0_frag = loadSPIRV(dev, "combined_image_sampler.frag.spv");
        auto sp1_frag = loadSPIRV(dev, "input_attachment.frag.spv");
        ...
        VkPipelineShaderStageCreateInfo sp0_frag_stage_info { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO };
        sp0_frag_stage_info.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
        sp0_frag_stage_info.module = sp0_frag;
        sp0_frag_stage_info.pName = "main";
        VkPipelineShaderStageCreateInfo sp1_frag_stage_info { VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO };
        sp1_frag_stage_info.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
        sp1_frag_stage_info.module = sp1_frag;
        sp1_frag_stage_info.pName = "main";
        std::array<VkPipelineShaderStageCreateInfo, 2> sp0_shader_stage_info { vert_stage_info, sp0_frag_stage_info };
        std::array<VkPipelineShaderStageCreateInfo, 2> sp1_shader_stage_info { vert_stage_info, sp1_frag_stage_info };
        ...
        std::array<VkGraphicsPipelineCreateInfo, 2> graphics_pipeline_info = {
            VkGraphicsPipelineCreateInfo {
                ...
                .pNext = nullptr,
                .flags = 0,
                .stageCount = (uint32_t)sp0_shader_stage_info.size(),
                .pStages = sp0_shader_stage_info.data(),
            ...
            VkGraphicsPipelineCreateInfo {
                ...
                .pNext = nullptr,
                .flags = 0,
                .stageCount = (uint32_t)sp1_shader_stage_info.size(),
                .pStages = sp1_shader_stage_info.data(),
                .pVertexInputState = &vert_input_state,
                .pInputAssemblyState = &ia_state,
                .pTessellationState = nullptr,
                .pViewportState = &vp_state,
    ...
    GfxImage2D *dst_texture; // Subpass 1 input attachment load dst_texture to swapchain
    VkSampler sampler;
};

}
int main() {
    common::TestInputattachment T;
    T.Gameloop();
    return 0;
}
