...
public:
    ...
    void InitGLFW() {
        ...
        window = glfwCreateWindow(800, 800, "multi descriptor", nullptr, nullptr);
    }
    void CreateInstance() {
    ...
    void BakeTexture() {
        {
            ...
            int w, h, c;
            uint8_t *ptr = stbi_load("../resource/vegeta.png", &w, &h, &c, STBI_rgb_alpha);
    ...
    void CreateGFXDescriptorSet() {
        ...
        }
        {
            std::vector<VkDescriptorSetLayoutBinding> bindings(1);
            bindings[0].binding = 0;
            bindings[0].descriptorType = VK_DESCRIPTOR_TYPE_SAMPLER; // GLSL f.inst "uniform sampler smp;"
            bindings[0].descriptorCount = 1;
            bindings[0].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
        ...
        alloc_info.descriptorSetCount = dsl.size();
    ...
    void CreatePipeline() {
        ...
        auto frag_spv = loadSPIRV("multi_descriptor_set.frag.spv");
        ...
        layout_info.setLayoutCount = dsl.size();
        layout_info.pSetLayouts = dsl.data();
    ...
    void BakeCommand() {
        for (uint32_t i = 0; i < cmdbuf.size(); i++) {
            ...
            vkCmdBindDescriptorSets(cmdbuf[i], VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, ds.size(), ds.data(), 0, nullptr);
