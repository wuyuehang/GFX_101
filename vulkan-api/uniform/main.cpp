...
class TestUniform : public SkeletonVulkan {
public:
    TestUniform() : SkeletonVulkan(VK_API_VERSION_1_0, 800, 800) {
        SkeletonVulkan::init();
        {
    ...
    void CreateBuffer() {
        ...
        ubo.view = glm::lookAt(glm::vec3(0.0, 0.0, -10.0), glm::vec3(0.0), glm::vec3(0.0, 1.0, 0.0));
        ubo.proj = glm::perspective(glm::radians(45.0), (double)m_width/m_height, 0.1, 10.0);

        uniform_buf = new GfxBuffer(this);
        uniform_buf->init(sizeof(MVP),VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        uniform_buf->host_update(&ubo, sizeof(MVP));
        uniform_buf->set_descriptor_info(0, sizeof(MVP));
    }
    void CreateDescriptor() {
        // descriptor set layout
        std::vector<VkDescriptorSetLayoutBinding> bindings = {
            GfxDescriptorSetLayoutBinding(0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT),
        };
        ...
        std::vector<VkDescriptorPoolSize> pool_size = {
            GfxDescriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1)
        };
        ...
        std::vector<VkWriteDescriptorSet> w = {
            GfxWriteDescriptorSet(ds, 0, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, &uniform_buf->get_descriptor_info())
        };
