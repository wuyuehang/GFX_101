...
public:
    ...
    void InitGLFW() {
        ...
        window = glfwCreateWindow(800, 800, "subpass", nullptr, nullptr);
    }
    void CreateInstance() {
    ...
    void BakeTexture() {
        int w, h, c;
        uint8_t *ptr = stbi_load("../resource/buu.png", &w, &h, &c, STBI_rgb_alpha);
    ...
    void BakeGFXDescriptorSet() {
        ...
        pool_size[0].descriptorCount = 2; // Subpass 0 & Subpass 1 consume two descriptors.
        ...
        VkDescriptorSetLayout pDSL[2] = { dsl, dsl }; // If allocates multiple descriptor sets, the descriptor set layout need to be arrays as well.
        alloc_info.pSetLayouts = pDSL;
    ...
    void CreatePipeline() {
        ...
        pipeline.resize(SPMAX);
        pipeline_info[SP0].sType = VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO;
        //pipeline_info.flags =
        pipeline_info[SP0].stageCount = shader_stage_info.size();
        pipeline_info[SP0].pStages = shader_stage_info.data();
        ...
        pipeline_info[SP0].layout = layout;
        ...
        pipeline_info[SP1].stageCount = shader_stage_info.size();
        pipeline_info[SP1].pStages = shader_stage_info.data();
        ...
        pipeline_info[SP1].layout = layout;
    ...
    void BakeCommand() {
        for (uint32_t i = 0; i < cmdbuf.size(); i++) {
            ...
            vkCmdBindDescriptorSets(cmdbuf[i], VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1, &ds[SP0], 0, nullptr);
            ...
            vkCmdBindVertexBuffers(cmdbuf[i], 0, 1, &mesh.buf, offsets);
            vkCmdBindDescriptorSets(cmdbuf[i], VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1, &ds[SP1], 0, nullptr);
            vkCmdDraw(cmdbuf[i], 6, 1, 0, 0);
...
private:
    ...
    bufobj mesh;
    texobj srctex;
    texobj dsttex; // Subpass 0's color render target, Subpass 1's shader read texture
    VkSampler sampler;
    VkPipelineLayout layout;
    std::vector<VkPipeline> pipeline; // can not share, it is per subpass
    VkDescriptorSetLayout dsl;
    VkDescriptorPool pool;
    std::vector<VkDescriptorSet> ds; // can not share, it is per subpass
};
