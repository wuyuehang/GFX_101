...
class TestSpecializationConstant : public SkeletonVulkan {
public:
    TestSpecializationConstant() : SkeletonVulkan(VK_API_VERSION_1_0, 800, 800) {
        {
    ...
    void CreateTexture() {
        ...
        texture->bake("../resource/frieza.png", VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
    ...
    void CreatePipeline() {
        ...
        auto frag = loadSPIRV(dev, "specialization_constant.frag.spv");
        ...
        struct specialization datum {
            .const_a = VK_TRUE,
            .const_b = VK_TRUE,
        };
        std::array<VkSpecializationMapEntry, 2> entry;
        entry[0].constantID = 0;
        entry[0].offset = offsetof(struct specialization, const_a);
        entry[0].size = sizeof(VkBool32);
        entry[1].constantID = 1;
        entry[1].offset = offsetof(struct specialization, const_b);
        entry[1].size = sizeof(VkBool32);
        VkSpecializationInfo spec_info {
            .mapEntryCount = entry.size(),
            .pMapEntries = entry.data(),
...
int main() {
    common::TestSpecializationConstant T;
    T.Gameloop();
    return 0;
}