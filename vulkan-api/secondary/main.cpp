...
public:
    ...
    void CreateTexture() {
        ...
        texture->bake("../resource/broly.png", VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
        texture->transition(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
        texture->set_descriptor_info(sampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    }
    void CreateDescriptor() {
        ...
        std::vector<VkWriteDescriptorSet> w = {
            GfxWriteDescriptorSet(ds, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &texture->get_descriptor_info())
        };
        vkUpdateDescriptorSets(dev, w.size(), w.data(), 0, nullptr);
    }
    void BakeCommand() {
        {
            VkCommandBufferAllocateInfo sec_cmdbuf_info { VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO };
            sec_cmdbuf_info.commandPool = cmdpool;
            sec_cmdbuf_info.level = VK_COMMAND_BUFFER_LEVEL_SECONDARY;
            sec_cmdbuf_info.commandBufferCount = 1;
            vkAllocateCommandBuffers(dev, &sec_cmdbuf_info, &sec_cmdbuf);

            VkCommandBufferInheritanceInfo inhe { VK_STRUCTURE_TYPE_COMMAND_BUFFER_INHERITANCE_INFO };
            inhe.renderPass = rp;
            inhe.subpass = 0;

            begin_secondary_cmdbuf(sec_cmdbuf, &inhe);

            vkCmdBindPipeline(sec_cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
            VkDeviceSize offsets[] { 0 };
            vkCmdBindVertexBuffers(sec_cmdbuf, 0, 1, &vertex_buf->get_buffer(), offsets);
            vkCmdBindDescriptorSets(sec_cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1, &ds, 0, nullptr);
            vkCmdDraw(sec_cmdbuf, 6, 1, 0, 0);

            end_cmdbuf(sec_cmdbuf);
        }
        ...
        for (uint32_t i = 0; i < cmdbuf.size(); i++) {
            ...
            vkCmdBeginRenderPass(cmdbuf[i], &begin_renderpass_info, VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS);
            vkCmdExecuteCommands(cmdbuf[i], 1, &sec_cmdbuf);
    ...
    GfxImage2D *texture;
    VkSampler sampler;
    VkCommandBuffer sec_cmdbuf;
};

}
int main() {
    common::TestSecondary T;
    T.Gameloop();
    return 0;
}