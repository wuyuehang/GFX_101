...
public:
    ...
    void InitGLFW() {
        ...
        window = glfwCreateWindow(800, 800, "secondary command", nullptr, nullptr);
    }
    void CreateInstance() {
    ...
    void BakeTexture() {
        {
            ...
            int w, h, c;
            uint8_t *ptr = stbi_load("../resource/broly.png", &w, &h, &c, STBI_rgb_alpha);
            assert(ptr && c == 4);
    ...
    void BakeSecondaryCommand() {
        ...
        cmdbuf_begin_info.pInheritanceInfo = &inhe; // Must be!!!
        vkBeginCommandBuffer(sec_cmdbuf, &cmdbuf_begin_info);

            vkCmdBindPipeline(sec_cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
            VkDeviceSize offsets[] { 0 };
            vkCmdBindVertexBuffers(sec_cmdbuf, 0, 1, &interleaved_buf, offsets);
            vkCmdBindDescriptorSets(sec_cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS, layout, 0, 1, &ds, 0, nullptr);
            vkCmdDraw(sec_cmdbuf, 6, 1, 0, 0);

        vkEndCommandBuffer(sec_cmdbuf);
    }
    void BakeCommand() {
