...
int main() {
    ...
    GLFWwindow *window = glfwCreateWindow(1024, 768, "gl blinn-phong", nullptr, nullptr);
    ...
    std::vector<std::string> shaders { "./blinn-phong.vert", "./blinn-phong.frag" };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBindVertexArray(VAO);
        P->use();
        P->setMat4("model_mat", mesh.get_model_mat());
        P->setMat4("view_mat", ctrl->get_view() * ctrl->get_model());
        P->setMat4("proj_mat", ctrl->get_proj());
        light_loc = glm::vec3(ctrl->get_view() * glm::vec4(0.0, 0.0, 3.0, 1.0));
        P->setVec3("light_viewspace", light_loc);
        mesh.draw(P);
