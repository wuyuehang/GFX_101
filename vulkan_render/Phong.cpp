...
void HelloVulkan::bake_phong_DescriptorSetLayout(VulkanPipe & pipe) {
    ...
    std::vector<VkDescriptorSetLayoutBinding> bindings {
        ...
        { 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_FRAGMENT_BIT }, // Light (location, intensity, etc)
    };
...
void HelloVulkan::bake_phong_DescriptorSet(VulkanPipe & pipe) {
    ...
    pool_size[1].descriptorCount = pipe.ds.size();
    ...
    for (auto i = 0; i < pipe.ds.size(); i++) {
        ...
        write[1].dstSet = pipe.ds[i];
        write[1].dstBinding = 1;
        write[1].dstArrayElement = 0;
        write[1].descriptorCount = 1;
        write[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        write[1].pBufferInfo = &light_buf_info;

        vkUpdateDescriptorSets(dev, write.size(), write.data(), 0, nullptr);
    }
}
